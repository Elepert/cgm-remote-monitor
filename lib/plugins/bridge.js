  ...
  function payload(err, glucose) {
    if (err) {
      ...
      if ('sbx' in ctx) {
        ...
        const lastPredictedTime = new Date(lastPredicted.startDate);
        ...
        if (timeDiff < 360000) {
          for (var i = 0; i < 24; i++) {
            if (lastPredicted.values[i] >= 130) {
              logger.info('Turn light yellow prediction');
              lightSet = true;
              break;
            }
          }
      ...
      entries.list({ count: 5 }, function (err, records) {
        if (err) {
          return;
        }
        if (records && !lightSet) {
          if (records[0].sgv < 70) {
            logger.info('Turn light red too low');
          } else if (records[0].sgv > 130) {
            logger.info('Turn light yellow over 130');
          } else if (records[0].sgv > 200) {
            logger.info('Flash light red too high');
          } else if (records[0].sgv > 120 && records[1].sgv > 120 && records[2].sgv > 120) {
            logger.info('Turn light yellow its been high');
